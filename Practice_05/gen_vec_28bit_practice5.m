...
input_bit = 28;    % input  vector bitwidth
output_bit = 56;   % output vector bitwidth

% generate random numbers and quantize them
q_A = uint64(floor(rand(1, N)*power(2, input_bit)));
q_B = uint64(floor(rand(1, N)*power(2, input_bit)));
...
int_Out = (int_A.*int_B);
Out_in_hex = fopen('./mult_output.txt', 'w');
for m = 1:N
    pr_Out = int_Out(m);
